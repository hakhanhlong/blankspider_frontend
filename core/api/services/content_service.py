...
class ContentService:
    ...
    def search(self,source_id='', tagid='', published_at='', keyword='', pageindex=1, pagesize=50):
        ...
        if source_id == '*' and tagid == '*' and published_at == '*' and keyword is not '*':
            self.requestHelpers.url = 'http://118.107.88.35:8983/solr/lcbc_search/select?fl=title,published_at,tag_name,version_count,published_time,id&indent=on&q='+keyword+'&rows='+str(pagesize)+'&start='+str(pageindex)+'&sort=title%20desc&wt=json'
        else:
